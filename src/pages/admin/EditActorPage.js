...
const EditActorPage = () => {
    ...
    const fetchActors = async () => {
        try {
            ...
            console.error("배우 목록 로딩 실패:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleEditClick = () => {
        ...
        setEditedPhotoUrl(selectedActor.photoUrl || '');
        setIsEditing(true);
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            await fetch(`/admin/actors/${selectedActor.id}`, {
                method: 'PUT',
                headers: {
    ...
    const handleDelete = async () => {
        if (!selectedActor || !window.confirm(`'${selectedActor.name}' 배우를 정말 삭제하시겠습니까?`)) return;
    ...
    return (
        <>
            ...
            <Container>
                <Title>배우 수정/삭제</Title>
                <ContentWrapper>
                    <SearchableSelect
                        options={actors}
                        value={selectedActor}
                        onChange={handleSelectActor}
                        placeholder="수정할 배우를 검색하세요..."
                    />
                    {selectedActor && (
                        <Card>
                            <CardImage src={isEditing ? editedPhotoUrl || 'https://placehold.co/150x150/e9ecef/adb5bd?text=Image' : selectedActor.photoUrl || 'https://placehold.co/150x150/e9ecef/adb5bd?text=Image'} alt={selectedActor.name} />
                            <CardBody>
                                <CardTitle>{selectedActor.name}</CardTitle>
                                {isEditing ? (
                                    <>
                                        <Input
                                            type="text"
